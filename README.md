...
- 无需额外安装 Python 依赖，拉取到 ComfyUI 自定义节点目录后即可使用
- 在 ComfyUI 节点卡片中直接填写 API Key
- 通过用量查询节点预览剩余额度和调用记录
...
安装完成后，重启 ComfyUI。无需执行 `pip install`，也不需要额外配置 Python 依赖。
...
推荐方式：直接在节点卡片里填写 `api_key`。

`MiaoXiaoHei Image to SVG`、`MiaoXiaoHei Download Result`、`MiaoXiaoHei API Usage` 都有 `api_key` 输入框。把后台发放的 API Key 填进去即可使用。

高级方式：如果你不想在每个节点里重复填写，也可以设置环境变量，然后节点里的 `api_key` 留空。
